...
app.get("/api/client/:client_name", (req, res) => {
  ...
  pool
    .query(`SELECT * FROM client WHERE first_name IN ($1);`, [client_name])
    .then((data) => {
...
app.get("/api/goals/:client_goals", (req, res) => {
  ...
  pool
    .query(
      `SELECT goals.client_id, client.first_name, goals.goals_id, goals.goal_name, 
    goals.complete_by, goals.priority, goals.complete FROM client RIGHT JOIN goals 
    ON goals.client_id=client.id WHERE first_name IN ($1)`,
      [client_goals]
    )
    .then((data) => {
...
app.post("/api/client", (req, res) => {
  ...
  if (!first_name || !last_name || !age || !phone_number || !email) {
    ...
    pool
      .query(
        `INSERT INTO client (first_name, last_name, age, phone_number, email) 
...
app.post("/api/goals/:client_goals", async (req, res) => {
  ...
  if (!client_goals || !goal_name || !complete_by || !priority || !complete) {
    ...
    const client_id = await pool.query(
      "SELECT id FROM client WHERE first_name = ($1);",
      [client_goals]
    );
    if (!client_id) {
      res.send(404);
      ...
      pool
        .query(
          `INSERT INTO goals (client_id, goal_name, complete_by, priority, complete)
       VALUES ($1, $2, $3, $4, $5) RETURNING *;`,
          [client_id.rows[0].id, goal_name, complete_by, priority, complete]
        )
        .then((data) => {
...
app.delete("/api/client/:id", (req, res) => {
  ...
  pool
    .query(`DELETE FROM client WHERE client_id = $1 RETURNING *;`, [id])
    .then((data) => {
...
app.delete("/api/goals/:id", (req, res) => {
  ...
  console.log(req.params);
  pool
    .query(`DELETE FROM goals WHERE complete = "yes" RETURNING *;`, [id])
    .then((data) => {
      if (data.rows.length === 0) {
        res.sendStatus(404);
      } else {
...
app.delete("/api/goals_completed", (req, res) => {
  ...
  console.log(req.params);
  pool
    .query(`DELETE FROM goals_complete WHERE complete = "yes" RETURNING *;`, [
      id,
    ])
    .then((data) => {
      if (data.rows.length === 0) {
...
app.delete("/api/goals_completed/:1", (req, res) => {
  ...
  console.log(req.params);
  pool
    .query(`DELETE FROM goals_complete WHERE client_id = $1 RETURNING *;`, [id])
    .then((data) => {
      if (data.rows.length === 0) {
        res.sendStatus(404);
      } else {
